...
function SearchModal({ onClose, onAdd, onAddMultiple, monitoredIds, addToast }) {
  ...
  const MAX_PAGINAS_DEPUTADO = 50; // teto de segurança: 5.000 proposições
  const loadDeputadoProposicoes = async (dep, role) => {
    ...
    setLoadProgress("");
    try {
      const { dados, links } = await getProposicoesByDeputado(dep.id, role, 1);
      ...
      // Total de páginas a partir do link "last"; fallback: seguir "next"
      const lastLink = links.find((l) => l.rel === "last");
      let totalPages = 1;
      if (lastLink) {
        const m = String(lastLink.href).match(/pagina=(\d+)/);
        if (m) totalPages = Math.min(parseInt(m[1], 10), MAX_PAGINAS_DEPUTADO);
      } else if (links.find((l) => l.rel === "next")) {
        totalPages = MAX_PAGINAS_DEPUTADO; // desconhecido: segue até acabar
      }
      ...
      for (let p = 2; p <= totalPages; p += 4) {
        ...
        setLoadProgress(`Carregando página ${nums[nums.length - 1]}${lastLink ? ` de ${totalPages}` : ""}... (${all.length} até agora)`);
        const pages = await Promise.all(nums.map((q) =>
          getProposicoesByDeputado(dep.id, role, q).catch(() => ({ dados: [], links: [] }))
        ));
        if (loadTokenRef.current !== token) return;
        let vazias = 0;
